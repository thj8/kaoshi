...
### 阶段 5：抢答引擎（1 天）✅
- [x] 管理员 `rush/start` → 状态 RUSHING，广播 `rush:start`（含 deadline/server_now）
- [x] `POST /api/question/:id/rush`：**Redis Lua 脚本**原子判序（ZSET+fail SET），只按服务器收到时间
- [x] 按配置取前 N 名抢答成功，其余 `rush:failed`；唯一第一名、不可重复抢、不可重复得分
- [x] 抢答成功者获得答题资格（可配置抢答奖励分，立即入账），进入答题倒计时（rush_answer_time）
- [x] 抢答窗口结束/名额满/管理员手动 → 广播 `rush:end`（含获答名单与答题 deadline）
- [x] 抢答题答错扣分（rush_wrong_score，可为负总分）；仅获答者可作答；未答获答者强制收卷记 "-"
- [x] 用户端大号抢答按钮四态（等待/可抢/成功/失败）+ 窗口结束态；获答者名单展示
- [x] 集成验证：50 并发抢答恰好 N 名获答、名次唯一稳定；WS 事件链全通

### 阶段 6：积分与实时排行榜（1 天）
- [x] 积分规则：普通题=题目分值；抢答题=抢答奖励 + 答对分 − 答错扣分（配置化）
- [x] 排行榜：分数变动（提交/抢答/reveal/rush:end）即 `ranking:update` 广播（排名/昵称/分数/正确题数），受 show_ranking 开关控制（MySQL participants 实时排序，ponytail: 不加 Redis ZSET 副本——分数经 SQL 原子累加，ZSET 会漂移，规模大时再引入）
- [x] 用户端实时排行榜组件（WS 推送浮动面板，不刷新页面）
- [x] 公布答案 `answer:reveal`：公共载荷广播 + 个人答案/得分按参与者单播（正确答案/解析受 quiz 配置开关控制；管理端含选项分布）

### 阶段 7：结束与统计（1 天）
- [x] `activity:end` → FINISHED，用户端成绩页：总分/题数/正确/正确率/用时/当前排名/最终排行榜
- [x] `GET /api/quiz/:id/result`
- [x] `GET /api/admin/quiz/:id/statistics`：参与数/完成数/平均分/最高最低分/平均正确率
- [x] 题目维度统计：答题人数/正确率/平均用时 → 发现难题
- [x] 管理端统计页面（`/admin/quiz/:id/stats`，总览卡片 + 题目正确率进度条 + 排行榜，5s 轮询）

### 阶段 8：加固与收尾（1 天）
- [x] 安全检查：JWT/WS 鉴权、只能提交自己答案、防重复提交计分、答案不下发（scripts/hardening_e2e.mjs 覆盖）
- [x] 并发测试：100 并发抢答 rank=1 唯一（E2E 实测通过）
- [x] 断线重连/刷新恢复回归（sync 恢复状态+当前题，重连后可继续作答）
- [x] UI 打磨（科技感、强反馈、移动端）、错误提示、空态（随各阶段实现，统计页含空态）
- [x] Docker Compose 一键启动跑通全流程（全部 E2E 均跑在 compose 服务上）
- [x] README（启动方式、演示流程、各阶段玩法说明、回归脚本）

### 阶段 9：考试模式增强与排行榜大屏（已完成）
- [x] 考试排行榜实时化：`examRanking` 从答题草稿聚合，未交卷也实时计分；
      同分排序：分数 DESC → 已交卷者在前 → 交卷/最后保存时间升序
- [x] 独立排行榜大屏 `/admin/rank/:id`（仅管理员）：深蓝大屏风、头部实时指标、
      🥇🥈🥉+分数条+交卷时间列、5s 轮询、手机自适应；控制台「🏆 排行榜」入口
- [x] 控制台考试模式右栏实时概况（人数/最高/最低/平均分/前10/逐题作答进度网格）
- [x] 防作弊：考试中排行榜/统计仅管理员可见（用户 REST 403，WS 不广播）
- [x] 考试用时口径修复：`duration_sec` = 本人首次作答 → 交卷的墙钟时间
      （不再逐题累加，避免切题窗口重叠虚高）
- [x] 考试页体验：交卷时间列、移除「标记」功能、题目标签字号加倍、
      当前题白色外圈高亮（已答蓝底时也清晰）
- [x] 回归：security_e2e（D1-D5）+ hardening_e2e（E22-E30）全绿
...
| M3（阶段5-6） | 抢答 + 排行榜 | 100 并发抢答第一名唯一，排行榜实时刷新 | ✅ 阶段5（50并发验证）+ 阶段6（E2E：reveal 单播个人答案/得分、ranking:update 实时广播） |
| M4（阶段7-8） | 统计 + 加固 + 部署 | Compose 一键启动，全流程可用 | ✅ 阶段7统计页 + 阶段8加固 E2E（越权/防重复/答案不泄露/100并发抢答唯一/断线重连）全通过 |
| M5（阶段9） | 考试排行榜实时化 + 大屏 + 用时口径修复 | 实时榜仅管理员可见、同分排序稳定、用时=首答→交卷墙钟 | ✅ security 50/50 + hardening 49/49 全绿 |
