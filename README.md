...
| 入口 | 地址 | 说明 |
|---|---|---|
| **答题用户端** | `http://<服务器IP>:13000/join/<答题ID>` | 点击链接输入昵称即加入（无邀请码） |
...
- **用户管理**：用户列表（参加场次/总得分/答题数/正确率/最近参与）、昵称搜索、改名、参加明细（每场成绩与排名）、删除（级联清理参与/答题/抢答记录）
...
3. **用户端**（手机/电脑浏览器打开链接）输入昵称即可，可开多个不同昵称模拟多人
