...
| **考试排行榜大屏** | `http://<服务器IP>:13000/admin/rank/<比赛码>` | 仅管理员（登录后访问）；深蓝大屏风，5s 自动刷新，适配投屏 |
...
✅ 阶段 0-9 全部完成：脚手架 / 数据模型 / 管理端 CRUD / 用户加入 + WS / 普通答题全流程 / 抢答引擎（Redis Lua 原子判序）/ 实时排行榜 + reveal 个人答案单播 / 结束统计页 / 加固验证 / 考试模式（自由切题全卷）/ 考试排行榜实时化 + 大屏 + 用时口径修复
...
覆盖：跨 quiz 越权 / 未参加提交 / 重复提交只计一次分 / 答案绝不下发 / **100 并发抢答 rank=1 唯一** / WS 断线重连 sync 恢复状态后可继续作答 / 考试实时榜权限与同分排序 / 考试用时口径（E30）。
...
1. 创建活动选「考试模式」并配置总时长；开考后全员拿到**同一份全卷**，可任意顺序作答
2. 每题选择即**自动保存**（服务端实时判分，可改答）；右上角倒计时走服务器时间，到点**统一收卷**
3. 交卷/收卷后出成绩页（得分/答对答错/正确率/用时/排名）。**用时口径**：本人首次作答 → 交卷的
   墙钟时间（服务端计算，与切题顺序/改答次数无关）
4. 排行榜**实时计分**（未交卷也上榜），同分排序：分数高在前 → 已交卷者在前 → 交卷/最后保存时间早者在前
5. 控制台右侧实时概况：人数/最高/最低/平均分/前 10 名（含交卷时间）/逐题作答进度网格
6. 防作弊边界：考试进行中用户端拿不到排行榜（REST 403、WS 不广播），仅管理员可见；结束时才广播最终榜单
